...
def create_line_plot(df, title):
    ...
    return LineChart(df, fig, "time", ["value"], title=title,
...
def create_histogram(df, title):
    ...
    return Histogram(df, fig, "value", 20, title=title, init_params={"Data": "value"})
...
def create_scatterplot(df, title):
    ...
    fig.layout.set_size(width=500, height=400)
    fig.layout.set_margin(left=40, right=40)
    fig.graphics.animate_on_load()
    ...
    # Make a histogram with 20 bins
    return ScatterPlot(df, fig, "time", "value", title=title,
...
def make_mg_layout(filename):
    ...
    # Make a FilterFrame add the button to the UI
    # ui.add_filter(btn)
    ...
    ui.add_chart(create_histogram(_stack, "PM forecast for the day of an area (time based)"))
    # ui.add_chart(create_histogram(_stack, "Journey forecast from A to B"))
    ui.add_chart(create_scatterplot(df, "Totals in relation to research mapped area"))
...
def get_layouts(mod, filename):
    ...
    _layouts["mg"] = {"layout": [mg_props], "title": "MakeZurich 2020"}

    register_layouts(_layouts, mod, brand="Breathe! Dashboard")
